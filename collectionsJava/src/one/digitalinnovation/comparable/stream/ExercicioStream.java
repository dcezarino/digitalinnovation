...
public class ExercicioStream {
	...
	public static void main(String[] args) {
		...
		System.out.println("Idade maior ou igual a 18: "
				+ estudantes.stream().filter((estudante) -> estudante.getIdade() >= 18).collect(Collectors.toList()));

		System.out.println(estudantes);

		// Retorna os elementos que tem b letra r no nome
		System.out.println("Com a letra B no nome: " + estudantes.stream()
				.filter((estudante) -> estudante.getNome().toLowerCase().contains("b")).collect(Collectors.toList()));
